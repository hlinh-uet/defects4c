...
| Test discovery | Parse `TESTLIST` | `ctest --show-only=json-v1` |
...
4. **Test discovery**: `ctest --test-dir build_meta_libyang --show-only=json-v1`
5. **Phase A buggy**: chạy toàn bộ CTest tuần tự để lấy `tests[*].outcome`
6. **Phase A fixed**: checkout fixed tree, build lại, chạy cùng danh sách test để lấy `tests[*].outcome_fixed`
7. **Phase B coverage**: checkout buggy tree, build lại, chạy toàn bộ CTest tuần tự; trước mỗi test xóa `.gcda`, sau test đọc coverage bằng `gcov`
...
| `tests[*].test_id` | CTest test name (e.g. `utest_new`) |
...
| `tests[*].covered_functions` | `gcov` parse thật → `"src/file.c:func"` |
