#!/usr/bin/env python3
"""Discover and run Apache Arrow tests at GoogleTest-case granularity."""
...
def ctest_entries(build_dir: Path, timeout: int) -> dict[str, CTestEntry]:
    ...
    for test in payload.get("tests", []):
        ...
        for prop in test.get("properties", []):
            ...
            elif prop.get("name") == "ENVIRONMENT":
                value = prop.get("value") or []
                if isinstance(value, str):
                    environment = tuple(part for part in value.split(";") if part)
                elif isinstance(value, list):
                    environment = tuple(str(part) for part in value if str(part))
        entries[name] = CTestEntry(name, command, working_directory, environment)
    if not entries:
...
def test_environment(entry: CTestEntry, build_dir: Path) -> dict[str, str]:
    env = os.environ.copy()
    env.setdefault("GTEST_COLOR", "no")
    root = build_dir.parent.parent
    env.setdefault("ARROW_TEST_DATA", str(root / "testing" / "data"))
    env.setdefault(
        "PARQUET_TEST_DATA",
        str(root / "cpp" / "submodules" / "parquet-testing" / "data"),
    )
    for assignment in entry.environment:
...
def list_gtest_cases(
    build_dir: Path, entry: CTestEntry, timeout: int
) -> list[str]:
    result = run(
        [*entry.command, "--gtest_list_tests"],
        cwd=entry.working_directory or None,
        env=test_environment(entry, build_dir),
        timeout=timeout,
    )
...
def test_ids(
    ...
    for target in targets:
        ...
        cases = list_gtest_cases(build_dir, entry, timeout)
...
def split_test_id(test_id: str) -> tuple[str, str]:
    ...
    if not target or (separator and not case):
        raise ValueError(f"Invalid Arrow test id: {test_id}")
    return target, case
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list_tests:
        for test_id in test_ids(build_dir, entries, args.list_tests, args.timeout):
